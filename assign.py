...
def read_power_traces(filename):
    power_traces = []
        ...
        for i in range(len(sample)):
            ...
            if len(power_trace) == NPoint:
                if (partial_cipher) & 1:
                    ...
                    sumBin0 += np.array(power_trace)
                    countBin0 += freqSample[i]

        # Calculate bias only if valid traces were added
        if countBin0 > 1e-10 and countBin1 > 1e-10:
            meanDiff = np.abs((sumBin0 / countBin0) - (sumBin1 / countBin1))
            bias = meanDiff.max()
            index = meanDiff.argmax()

            biasKey[key] = bias
            biasIndex[key] = index

    return biasKey, biasIndex
...
# Read power traces from CSV file
power_traces = read_power_traces('HW_power_trace_1.csv')
...
# Assume equal frequency for all samples
...
# Recover the key bytes with the highest bias value
recovered_key_byte_4 = np.argmax(biasKey4)
recovered_key_byte_5 = np.argmax(biasKey5)
